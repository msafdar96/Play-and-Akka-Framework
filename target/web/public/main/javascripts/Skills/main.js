...
const skillSocket = new WebSocket("ws://localhost:9000/freelancelot/skills");
const descriptionReadabilitySocket = new WebSocket("ws://localhost:9000/freelancelot/descriptionReadability");
skillSocket.onopen = function (e) {
    console.log("Skill Socket Connected!");
    const keyword = window.location.pathname.split("/")[3].replaceAll("-"," ")
    search(keyword);
};
...
skillSocket.onclose = function (event) {
    if (event.wasClean) {
...
skillSocket.onerror = function (error) {
...
const search = (keyword) => {
    ...
    skillSocket.send(message);
    skillSocket.onmessage = (e) => {
